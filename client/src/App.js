...
const SecureVoiceApp = () => {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      // TODO: Восстановление состояния комнаты из sessionStorage
      const savedRoomState = sessionStorage.getItem('currentRoomState');
      if (savedRoomState) {
        try {
          const roomState = JSON.parse(savedRoomState);
          console.log('🏠 Восстанавливаем состояние комнаты:', roomState);
          setCurrentRoom(roomState.room);
          setParticipants(roomState.participants);
          setCurrentView(roomState.view);
        } catch (error) {
          console.error('❌ Ошибка восстановления комнаты:', error);
      ...
      if (savedName) {
        ...
        try {
          ...
          if (sessionData && sessionData.user) {
            ...
            // TODO: Если есть сохраненная комната, подключаемся к WebSocket
            if (savedRoomState && savedRoomState.room) {
              console.log('🔌 Переподключаемся к WebSocket комнаты');
              connectWebSocket(savedRoomState.room.id, sessionData.user.id);
            } else {
              setCurrentView('main');
            }
            ...
            // TODO: Запрашиваем разрешения на уведомления
  ...
  const requestMicrophonePermission = useCallback(async () => {
    try {
      console.log('🎤 Запрос разрешения на микрофон...');
      ...
      if (navigator.permissions) {
        try {
          ...
          console.log('⚠️ Не удалось проверить разрешения:', permError.message);
        }
      }
      ...
      if (error.name === 'NotAllowedError') {
        errorMessage += 'Разрешите доступ к микрофону в браузере и попробуйте снова.';
  ...
  const initializeMicrophoneForRoom = useCallback(async () => {
    ...
    try {
      ...
      console.log('🎤 Микрофон успешно инициализирован для комнаты');
      
    } catch (error) {
      console.error('❌ Не удалось инициализировать микрофон:', error);
      // Не показываем alert, так как пользователь уже был уведомлен
    }
  }, [currentView, requestMicrophonePermission]);
  ...
  const testMicrophone = useCallback(async () => {
    ...
    try {
      console.log('🧪 Начинаем тест микрофона...');
      const stream = await requestMicrophonePermission();
      startMicrophoneMonitoring(stream);
      
      setTimeout(() => {
        console.log('⏰ Завершаем тест микрофона');
        setIsTestingMicrophone(false);
        stream.getTracks().forEach(track => track.stop());
        
        // Сбрасываем уровень после теста
  ...
  }, [requestMicrophonePermission, startMicrophoneMonitoring]);
  ...
  const connectWebSocket = useCallback((roomId, userId) => {
    ...
    ws.onmessage = (event) => {
      ...
      switch (data.type) {
        ...
        case 'join_request':
          // TODO: Показываем уведомление только создателю
          if (currentUser?.is_creator) {
            console.log('📨 Новый запрос на подключение:', data.user);
            ...
            // TODO: Звуковое уведомление
            if ('Notification' in window && Notification.permission === 'granted') {
              new Notification('SecureVoice', {
                body: `${data.user.name} хочет присоединиться к комнате`,
                icon: '/favicon.ico'
              });
            }
          }
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      const requestHeaders = sessionManager.getAuthHeaders();
      ...
      // TODO: Сохраняем состояние комнаты в sessionStorage
      const roomState = {
        room: data.room,
        participants: [data.user],
        view: 'pre-call'
      };
      sessionStorage.setItem('currentRoomState', JSON.stringify(roomState));
      
      // Подключаемся к WebSocket
      console.log('🔌 Подключаемся к WebSocket...');
      connectWebSocket(data.room_id || data.room.id, data.user.id);
      ...
      console.log('🔄 Обновляем списки комнат...');
      await loadRooms();
      await loadUserRooms();
      
      // Переходим к предзвонку
      console.log('🎤 Переходим в режим предзвонка');
      setCurrentView('pre-call');
  ...
  const joinRoom = async (roomId) => {
    console.log('🚪 Начинаем присоединение к комнате:', roomId);
    ...
    console.log('🔍 Проверяем существование комнаты...');
    const roomExists = await checkRoomExists(roomId);
    if (!roomExists) {
      console.error('❌ Комната не существует:', roomId);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          errorMessage = 'Неверный пароль для входа в комнату';
        } else if (response.status === 403) {
          errorMessage = 'Доступ к комнате запрещен';
      ...
      // Определяем следующий шаг на основе ответа сервера
      let nextView;
      if (data.awaiting_approval) {
        console.log('⏳ Ожидаем одобрение создателя...');
        setAwaitingApproval(true);
        nextView = 'waiting-approval';
        setCurrentView('waiting-approval');
        connectWebSocket(roomId, data.user.id);
      } else if (data.in_waiting_room) {
        console.log('🚪 Помещены в зал ожидания...');
        nextView = 'waiting-room';
        setCurrentView('waiting-room');
        connectWebSocket(roomId, data.user.id);
      } else {
        console.log('🎤 Входим напрямую в комнату...');
        nextView = 'pre-call';
        setCurrentView('pre-call');
        connectWebSocket(roomId, data.user.id);
      }
  ...
  const endCall = () => {
    ...
    // TODO: Очищаем сохраненное состояние комнаты
    sessionStorage.removeItem('currentRoomState');
    setCurrentView('main');
  };
  ...
  const logout = () => {
    ...
    // TODO: Очищаем все сохраненные состояния
    sessionStorage.removeItem('currentRoomState');
  };
        ...
        <div className="bg-gray-800 rounded-lg p-6 mb-6">
          <h3 className="font-medium mb-4">Проверка устройств</h3>
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                {isAllMuted ? <VolumeX size={16} /> : <Volume1 size={16} />}
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="flex-1 flex flex-col p-6">
        ...
        <div className="flex items-center justify-center space-x-4">
          ...
          >
            {isAllMuted ? <VolumeX size={20} /> : <Volume1 size={20} />}
          </button>
